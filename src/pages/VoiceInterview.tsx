...
import { Mic, MicOff, Play, Square, Volume2, RefreshCw, CheckCircle, Send } from 'lucide-react';
...
export const VoiceInterview = () => {
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      // Continue even if there's an error (e.g. denied permissions)
    }
  ...
  const handleTextSubmit = () => {
    ...
    if (!trimmed || !isActive || isSpeaking) return;
    ...
    <div className="max-w-4xl mx-auto h-[calc(100vh-140px)] flex flex-col gap-6">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
        ...
        <div className="lg:col-span-1 bg-slate-900 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden shadow-xl text-white">
          ...
          {/* Visuals / Camera */}
          <div className="absolute inset-0 p-4 opacity-50 z-0 pointer-events-none">
            <video
              ref={videoRef}
              autoPlay
              muted
              playsInline
              className="w-full h-full object-cover rounded-xl filter grayscale contrast-125 mix-blend-overlay"
            />
          </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 flex flex-col overflow-hidden shadow-sm">
          ...
          {/* Text Input Bar */}
          {!showFeedback && (
            <div className={`p-4 border-t border-slate-200 bg-white flex items-center gap-3 transition-opacity ${!isActive ? 'opacity-50 pointer-events-none' : ''}`}>
              <input
                id="voice-interview-text-input"
                type="text"
                value={textInput}
                onChange={e => setTextInput(e.target.value)}
                onKeyDown={handleKeyDown}
                disabled={!isActive || isSpeaking}
                placeholder={isActive ? (isSpeaking ? "Wait for the AI to finish..." : "Type your answer here...") : "Start the interview to answer"}
                className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-all disabled:cursor-not-allowed"
              />
              <button
                id="voice-interview-send-btn"
                onClick={handleTextSubmit}
                disabled={!isActive || isSpeaking || !textInput.trim()}
                className="p-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl transition-all flex-shrink-0"
              >
              ...
              <div className="flex flex-col gap-4 w-full">
                ...
                {recordingUrl && (
                  <div className="mt-6 border-t border-green-200 pt-6">
                    <h4 className="font-bold text-green-800 mb-3">Interview Recording</h4>
                    <video src={recordingUrl} controls className="w-full max-w-sm rounded-xl border border-green-200 shadow-sm bg-black" />
                    <div className="mt-4">
                      <a
                        href={recordingUrl}
